...
const symbols = ['BTCUSDT', 'ETHUSDT', 'SUIUSDT', 'DOGEUSDT', 'TRBUSDT']; // 可擴充為 BingX 合約所有幣種
const interval = '4h'; // 分析週期

async function fetchKlines(symbol) {
  const url = `https://api.binance.com/api/v3/klines?symbol=${symbol}&interval=${interval}&limit=100`;
  const response = await axios.get(url);
  return response.data.map(k => parseFloat(k[4])); // 收盤價
}
...
function analyzeSymbol(prices) {
  ...
  let trend = '震盪';
  if (rsi > 60 && macd.histogram[macd.histogram.length - 1] > 0 && currentPrice > vegasShort && currentPrice > vegasLong) {
    trend = '偏多';
  } else if (rsi < 40 && macd.histogram[macd.histogram.length - 1] < 0 && currentPrice < vegasShort && currentPrice < vegasLong) {
    trend = '偏空';
  }
